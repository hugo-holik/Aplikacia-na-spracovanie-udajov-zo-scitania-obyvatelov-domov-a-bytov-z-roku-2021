...
namespace structures
{
	...
	{
		// ked chces spustit indexOfKey() (bisekciu), tak do indexStart das 0 a do indexEnd das size()-1
		// lenze ak tabulka je prazdna (size() == 0), tak size()-1 == -1 ... pre toto je spraveny tento if
		if (indexEnd == -1) 
		{
			found = false;
			return 0;
		...
		else
		{
			...
			else
			{
				...
				{
					return indexOfKey(key, indexStart, indexMiddle, found); // tu ma varga indexMiddle - 1 ... neviem ci je tak dobre (ze aby nemusel cekovat uz ceknuty key na middleIndexe)
				}
			}
		}
	}
