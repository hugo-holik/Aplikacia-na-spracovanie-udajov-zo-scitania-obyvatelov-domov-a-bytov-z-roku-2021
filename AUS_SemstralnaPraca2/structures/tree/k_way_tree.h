...
namespace structures
{
	...
	{
		//prechadzku rekurzivne PostOrderom. lebo nemozes najprv zahodit seba -> nedostal by si sa k synom
		for (int i = 0; i < K; i++)
		{
			delete children_->at(i); // ak syn bude nullptr a deletnes nullptr -> tak sa nic nestane, validna operacia
		}
	...
	{
		for (int i = 0; i < K; i++) // nemusis i < children->size(), staci i < K (bude rychlejsie)
		{
	...
	{
		// insert najprv vymaze co bolo na order, a vlozi tam noveho. (nevtesna dalsie syna medzi ostatnych)
		//throw std::logic_error(" KWayTreeNode<T, K>::insertSon is not supported!");
		//delete replaceSon(son, order);
		//?? este treba vlozit TODO (pozri cviko obs)

		throw std::logic_error("KWayTreeNode<T>::insertSon: is not supoerted");
	}
	...
	inline TreeNode<T>* KWayTreeNode<T, K>::replaceSon(TreeNode<T>* son, int order)
	{
		...
		//// ked chcem vymazat syna, tak vlozim do syna nullptr
		//// tento if je nato aby sme vedeli vlozit do children_ aj nullptr, a nullptr nechces/nemozes posielat spravu setParent
		//if (son != nullptr)
		//{
		//	son->setParent(this);
		//}
	...
	inline int KWayTreeNode<T, K>::degree()
	{
		...
		////v children mozu byt diery ( npr pre K = 3: moze mat laveho syna, moze mat praveho, nemusi mat stredneho(stredny = nullptr )
