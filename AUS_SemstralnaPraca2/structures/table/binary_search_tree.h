...
namespace structures
{
	...
	{
		//TODO: pozri si obs - AUS cv11 - tabulky2 ... na case: 39:03
		// tento assign je komplikovany, a je to dobre vysvetlene

		// nemozes prejst BST for eachom - lebo vo for eachi je InOrder
		// nemozes navkladat prvky InOrderom - ich ti bude vkladat v poradi klucov
		// a zdegeneruje ti BST vzdy na zoznam.
		// mozes pouzit PreOrder alebo LevelOrder (spravia skoro to iste)
		// tie ti zrekonstrujuju presnu kopiu other (podla hierarchie)
		...
		if (this != &other)
		{
			...
			auto itB = new Tree<TableItem<K, T>*>::PreOrderTreeIterator(otherBST.binaryTree_->getRoot());
			auto itE = new Tree<TableItem<K, T>*>::PreOrderTreeIterator(nullptr);
	...
	{
		BSTTreeNode* nodeToReplace = nullptr;
		...
		{
			nodeToReplace = node->getRightSon();

			while (nodeToReplace->hasLeftSon()) 
			{
				nodeToReplace = nodeToReplace->getLeftSon();
			}

			extractNode(nodeToReplace);
			nodeToReplace->changeRightSon(node->changeRightSon(nullptr));
			nodeToReplace->changeLeftSon(node->changeLeftSon(nullptr));
			
		}

		if (node->isRoot()) 
		{
			binaryTree_->replaceRoot(nodeToReplace);
		}
		else 
		{
			if (node->isRightSon())
			{
				parent->changeRightSon(nodeToReplace);
			}
			else
			{
				parent->changeLeftSon(nodeToReplace);
			}
