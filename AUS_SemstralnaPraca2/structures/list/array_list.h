...
namespace structures
{
	...
	{
		//sprav test identity (lebo mozes deletnut sameho seba)
		//pretypuj
		//zahod svoje pole -> nemozes len priradit druhy other.array_, lebo array_ moze mat ine rozmery (enlargovanie arrayov), preto treba najprv povodny array_ zahodit a az potom donho priradit novy (kopiu other.array_) 
		//prirad si nove pole, kt je vytvorene kopiou pola z other
		// prirad size
		if (this != &other)
		{
	...
	{
		//skontroluj co je v array_ ale iba po size_, lebo v Array<T> moze byt za size_ bordel (ArrayList spravuje len urcitu cast Arrayu)
		if (this != &other)
		{
			ArrayList<T>* otherArrayList = dynamic_cast<ArrayList<T>*>(&other);
			if (otherArrayList != nullptr)
			{
				if (size_ != otherArrayList->size())
				{
					return false;
				}
				// teraz idem porovnat prvky
				// najrychlejsie by bolo porovnat array_ a otherArrayList->array_ -> nemusi byt SPRAVNE!!!
				// pretoze array_ sa pocas zivotneho cyklu ArrayListu moze zvacsovat/zmensovat alebo
				// mat na poziciach, kt nespravuje ArrayList neplatne hodnoty
				// takze treba skontrolovat kazdy prvok ArrayListu cez at(i)
				for (int i = 0; i < size_; i++) 
				{
					if (array_->at(i) != otherArrayList->at(i))
					{
						return false;
					}
				}
	...
	{
		T result = array_->at(index); // array_->at(index) uz kontroluje indexy
		if (index != size_ - 1)
		{
	...
	{
		return new ArrayListIterator(this, 0); // index 0 je prvy platny prvok v arrayliste
	}

	template<typename T>
	inline Iterator<T>* ArrayList<T>::getEndIterator()
	{
		return new ArrayListIterator(this, size_); //index size_ je prvy neplatny prvok v arrayliste
	}

	template<typename T>
	inline void ArrayList<T>::enlarge()
	{
		//vytvor vacsi array(size*2)
		//prekopiruj prvky (pouzi copy triedy Array)
		//zahod array_
		//do array_ zarad ten vacsi array
	...
	inline Iterator<T>& ArrayList<T>::ArrayListIterator::operator=(Iterator<T>& other)
	{
		...
		arrayList_ = otherIterator.arrayList_; //je nutne?
		position_ = otherIterator.position_;
		return *this; //toto som pridal
	}
	...
	{
		ArrayListIterator* otherIterator = dynamic_cast<ArrayListIterator*>(&other);
		return otherIterator == nullptr || arrayList_ != otherIterator->arrayList_ || position_ != otherIterator->position_;
	}
	...
	{
		++position_;	// to iste co position_++, iba sa to inac sprava v zlozitejsich vyrazoch
		return *this;
	}
