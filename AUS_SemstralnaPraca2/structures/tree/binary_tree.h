...
namespace structures
{
	...
	inline BinaryTreeNode<T>* BinaryTreeNode<T>::getParent()
	{
		...
		return dynamic_cast<BinaryTreeNode<T>*>(KWayTreeNode<T, 2>::getParent()); // KWayTreeNode<T>::getParent() -> volanie metody predka
	}

	template<typename T>
	inline BinaryTreeNode<T>* BinaryTreeNode<T>::getLeftSon()
	{
		//KWayTreeNode<T, 2>::getSon() -> takto sa vola v C++ metoda predka (ekvivalent super.getSon() v jave (asi))
	...
	inline void BinaryTreeNode<T>::setLeftSon(BinaryTreeNode<T>* leftSon)
	{
		...
		//alebo TOTO AK SI v insertSon hadzal vynimku
	...
	inline void BinaryTree<T>::InOrderTreeIterator::populatePath(TreeNode<T>* current)
	{
		 if (current != nullptr)
		 {
			 BinaryTreeNode<T>* currentBTN = dynamic_cast<BinaryTreeNode<T>*>(current);
																// inorder:
			 populatePath(currentBTN->getLeftSon());   //lavy
		     TreeIterator::path_->push(current);			  //seba
			 populatePath(currentBTN->getRightSon());  //pravy
		 }
	}
