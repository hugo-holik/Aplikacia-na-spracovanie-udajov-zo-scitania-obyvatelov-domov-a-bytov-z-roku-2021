...
namespace structures
{
	...
	{
		// insert je O(n) -> lebo robim kontrolu unikatnosti klucu
		// ale mozes toto pretazit, a nekontrolovat unikatnost klucu
		// (ak vies ze vkladany je na 100% unikatny) a stane za z tohto O(1)
	...
	{
		// bezpecny find: boolom indikujem ci sa naslo/nenaslo (nevyhadzujem vynimku -> bezpecna operacia)
		// ked je to bezpecne (ziadne vynimky), tak nemusim pisat try catch bloky
		// parameter data: cez referenciu naplnim toto pamatove miesto (output tryFindu ... ako navratova hodnota find())
		// parameter key: pomocou neho vyhladavam (ako vo find())
	...
	{
		// prejdi tabulku foreachom, ak nic kluc nenajdes -> return nullptr
		for (auto item : *list_)
		{
			if (item->getKey() == key)
			{
				return item; //tu neviem ci ma byt este item->accessData();
			}
		}
